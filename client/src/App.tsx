...
export default function App() {
  ...
  return <div className={`app-shell cloudburst-app ${running ? 'race-active' : ''}`}>
    ...
    <main>
      <div className="page-intro"><div><div className="eyebrow"><span className="tiny-checkers" /> THE APEX TOUR / SINGLE PLAYER GRAND PRIX</div><h1>Your next great rivalry<span>.</span></h1><p>Seven rivals. Three worlds. Your championship starts here.</p></div><div className="intro-stamp"><span>THE FORECAST?</span><strong>a chance of chaos.</strong><span>☁ &nbsp; ⚡ &nbsp; ☀</span></div></div>
      {!running && <><TrackSelect selected={courseId} onSelect={setCourseId} /><div className="race-mode-picker" aria-label="Race mode"><button aria-pressed={mode === 'single'} onClick={() => setMode('single')}>Single race</button><button aria-pressed={mode === 'cup'} onClick={() => setMode('cup')}>Three-race championship</button><p>{mode === 'cup' ? 'Cloudburst → Neon Night Market → Stormwater. 15 / 12 / 10 / 8 / 6 / 4 / 2 / 1 points. DNFs score 0.' : 'Race your selected course. Records and ghosts are saved separately for each track.'}</p></div></>}
      <div className="race-layout">
        <section className="circuit-panel">
          ...
          <div className="track-info"><div className="track-number">0{CUP_TRACKS.indexOf(courseId)+1}</div><div><h2>{track.name}<span>{mode === 'cup' ? `APEX TOUR · ${CUP_TRACKS.indexOf(courseId)+1}/3` : track.name.toUpperCase()}</span></h2><p>{track.subtitle}</p></div><div className="track-details"><span><strong>{Math.round(track.courseLength)} m</strong>TRACK LENGTH</span><span><strong>3 laps</strong>8 RACERS</span><span><strong>4 power-ups</strong>MAXIMUM MISCHIEF</span></div></div>
        </section>
