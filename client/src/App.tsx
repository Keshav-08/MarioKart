import { VEHICLES, ADVENTURES, type Vehicle } from './game/adventure';
...
export default function App() {
  ...
  const [vehicle, setVehicle] = useState<Vehicle>('kart');
  ...
  return <div className={`app-shell cloudburst-app ${running ? 'race-active' : ''}`}>
    ...
    <main>
      ...
      <div className="race-layout">
        <section className="circuit-panel">
          <div className="game-stage" ref={stage}>
            <GameCanvas vehicle={vehicle} courseId={courseId} color={color} name={name.trim() || 'Racer 01'} difficulty={difficulty} raceKey={raceKey} running={running} paused={paused} audio={audio.current} ghost={ghostEnabled ? ghost : []} onTelemetry={setTelemetry} onFinish={finish} onReset={reset} onPause={() => setPaused(value => !value)} />
          ...
          <div className="track-info"><div className="track-number">0{ALL_TRACKS.indexOf(courseId)+1}</div><div><h2>{track.name}<span>{mode === 'cup' ? `APEX TOUR · ${CUP_TRACKS.indexOf(courseId)+1}/3` : track.name.toUpperCase()}</span></h2><p>{track.subtitle}</p></div><div className="track-details"><span><strong>{Math.round(track.courseLength)} m</strong>TRACK LENGTH</span><span><strong>3 laps</strong>8 RACERS</span><span><strong>9 power-ups</strong>MAXIMUM MISCHIEF</span></div></div>
        </section>
        <aside className="setup-panel">
          {running ? <><div className="setup-heading"><span className="eyebrow">{mode === 'cup' ? `APEX TOUR · ${CUP_TRACKS.indexOf(courseId)+1}/3` : track.name.toUpperCase()}</span><h2>{finished ? 'The finish order.' : 'The running order.'}</h2></div><div className="prix-standings">{telemetry?.standings.map((racer, index) => <div className={racer.id === 0 ? 'is-you' : ''} key={racer.id}><b>{index + 1}</b><i style={{ background: racer.color }} /><span>{racer.name}{racer.id === 0 && <small>YOU</small>}</span><em>{racer.finish !== null ? formatTime(racer.finish * 1000) : telemetry.complete ? 'DNF' : `LAP ${racer.lap}`}</em></div>)}</div><div className="race-side-tip"><Zap size={20} /><p>{telemetry?.item ? ITEMS[telemetry.item].description : 'Grab a floating cube to get a power-up. Press E or Shift to use it.'}</p></div><button className="secondary-button" onClick={reset}><RotateCcw size={15} />Return to paddock</button><p className="start-hint">C · camera &nbsp; P / Esc · pause &nbsp; R · reset</p></> : <>
            ...
            <div className="field-label color-label">YOUR VEHICLE<span>WINGS INCLUDED</span></div><div className="vehicle-picker">{Object.entries(VEHICLES).map(([id,v])=><button key={id} aria-pressed={vehicle===id} onClick={()=>setVehicle(id as Vehicle)}><strong>{v.name}</strong><small>{v.description}</small></button>)}</div><p className="difficulty-hint">Automatic plane transformation at {ADVENTURES[courseId].flightName.toLowerCase()}. Steer with A/D; Q climbs and F dives.</p>
      ...
      <footer><span className="footer-brand">apex <span>KART CLUB</span></span><span>Wheels. Wings. Rivalries.</span><span>SEE YOU ON THE PODIUM <Flag size={12} /></span></footer>
    </main>
    <dialog ref={dialog} onCancel={() => setModal(null)} onClick={event => { if (event.target === event.currentTarget) setModal(null); }} aria-label={modal === 'help' ? 'How to play' : 'Personal race records'}><div className="modal-content"><button className="modal-close" aria-label="Close dialog" onClick={() => setModal(null)}><X size={20} /></button>{modal === 'help' ? <><span className="eyebrow">WELCOME TO {mode === 'cup' ? `APEX TOUR · ${CUP_TRACKS.indexOf(courseId)+1}/3` : track.name.toUpperCase()}</span><h2>Eight racers. One podium.</h2><p>Beat seven bots around three laps of your selected course, or race all three in the Apex Tour championship. They follow the road, overtake, collide, and use the same items you do.</p><div className="help-controls">{[['W / ↑', 'Accelerate'], ['S / ↓', 'Brake, then reverse'], ['A D / ← →', 'Steer'], ['SPACE', 'Hold while turning to charge a drift. Release to mini-turbo.'], ['E / SHIFT', 'Use an item or spend one turbo charge'], ['Q / F', 'In plane mode: climb / dive. W accelerates, S slows down.'], ['P / ESC', 'Pause or resume the whole race'], ['C', 'Switch chase / overview camera'], ['R', 'Return to paddock; discard the current race']].map(([key, description]) => <div key={key}><kbd>{key}</kbd><span>{description}</span></div>)}</div><p>Floating cubes respawn five seconds after collection. Golden arrow pads boost you. The skybridge pad launches a jump. The rough observatory shortcut is fastest with a turbo.</p><p>Shields block one attack or expire after six seconds. Getting hit briefly spins you; you can still recover and win. Your best races and one full-race ghost are saved in this browser.</p><button className="primary-button" onClick={() => setModal(null)}>Got it. Let’s race. <ArrowRight size={16} /></button></> : <><span className="eyebrow">{COURSES[recordTrackId].name.toUpperCase()} / THIS BROWSER</span><h2>Your fastest flights.</h2><div className="record-track-tabs">{ALL_TRACKS.map(id => <button key={id} aria-pressed={recordTrackId===id} onClick={() => setRecordTrackId(id)}>{COURSES[id].name}</button>)}</div>{!visibleRecords.length ? <p>Finish your first Grand Prix to save a record and unlock your personal-best ghost.</p> : <div className="record-list">{visibleRecords.map((r, i) => <div key={r.id}><b>{String(i + 1).padStart(2, '0')}</b><span>{r.name}<small>{r.vehicle ? VEHICLES[r.vehicle].name : 'Kart'} · {r.difficulty} · FINISHED #{r.position}</small></span><strong>{formatTime(r.time * 1000)}</strong></div>)}</div>}</>}</div></dialog>
  </div>;
}
