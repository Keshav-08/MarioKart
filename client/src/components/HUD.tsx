...
export default function HUD({ telemetry, courseId }: { telemetry: Telemetry | null; courseId: CourseId }) {
  ...
  return <div data-motion={telemetry?.motion} className={`hud prix-hud ${racing ? 'is-racing' : ''}`}>
    ...
    {racing && <>
      ...
      <div className="race-mini-map"><svg viewBox={courseId === 'beach' ? '-15 -15 210 200' : '0 0 178 166'} aria-label="Course minimap">{courseId === 'beach' && <ellipse cx='87.5' cy='80' rx='95' ry='95' fill='#f5d28b44'/>}<polyline points={mapPoints} fill="none" stroke="#ffffff40" strokeWidth="7" strokeLinejoin="round" />{courseId === 'beach' && (() => { const gate=track.pointAt(telemetry.nextCheckpoint/16); return <circle cx={(gate.x+125)*.7} cy={(gate.z+95)*.7} r={5} fill="none" stroke="#ffe187" strokeWidth={2}/>; })()}{[...telemetry.standings].reverse().map(r => { const p = r.id === 0 ? { x: telemetry.x, z: telemetry.z } : course.getPointAt(((r.progress % 1) + 1) % 1); return r.id === 0 ? <g key={r.id} className="player-map-marker" transform={`translate(${(p.x + 125) * .7},${(p.z + 95) * .7}) rotate(${-telemetry.yaw * 180 / Math.PI})`}><path d="M 0 6 L -4 -4 L 4 -4 Z" fill={r.color} stroke="#fff" strokeWidth="1.5" /></g> : <circle key={r.id} cx={(p.x + 125) * .7} cy={(p.z + 95) * .7} r={2.7} fill={r.color} />; })}</svg></div>
