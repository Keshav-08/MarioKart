import { WORLD_SCALE } from '../game/beach';
...
export default function HUD({ telemetry, courseId }: { telemetry: Telemetry | null; courseId: CourseId }) {
  ...
  const mapPoints = useMemo(()=>course.getSpacedPoints(150).map(p => `${(p.x/WORLD_SCALE + 125) * .7},${(p.z/WORLD_SCALE + 95) * .7}`).join(' '),[course]);
  ...
  return <div data-motion={telemetry?.motion} className={`hud prix-hud ${racing ? 'is-racing' : ''}`}>
    ...
    {racing && <>
      ...
      <div className={`item-slot ${item ? 'loaded' : ''}`} style={{ '--item-color': item?.color ?? '#a8bfdf' } as React.CSSProperties}><span className="item-icon">{item?.icon ?? '?'}</span><div><strong>{item ? item.name + (telemetry.charges>1?` ×${telemetry.charges}`:'') : 'GRAB AN ITEM BOX'}</strong><small>{item ? 'E / SHIFT TO USE' : 'Drive through a floating cube'}</small></div></div>
      <div className="race-mini-map"><svg viewBox={courseId === 'beach' ? '-15 -15 210 200' : '0 0 178 166'} aria-label="Course minimap">{courseId === 'beach' && <ellipse cx='87.5' cy='80' rx='95' ry='95' fill='#f5d28b44'/>}<polyline points={mapPoints} fill="none" stroke="#ffffff40" strokeWidth="7" strokeLinejoin="round" />{courseId === 'beach' && (() => { const gate=track.pointAt(telemetry.nextCheckpoint/16); return <circle cx={(gate.x/WORLD_SCALE+125)*.7} cy={(gate.z/WORLD_SCALE+95)*.7} r={5} fill="none" stroke="#ffe187" strokeWidth={2}/>; })()}{[...telemetry.standings].reverse().map(r => { const p = r.id === 0 ? { x: telemetry.x, z: telemetry.z } : course.getPointAt(((r.progress % 1) + 1) % 1); return r.id === 0 ? <g key={r.id} className="player-map-marker" transform={`translate(${(p.x/WORLD_SCALE + 125) * .7},${(p.z/WORLD_SCALE + 95) * .7}) rotate(${-telemetry.yaw * 180 / Math.PI})`}><path d="M 0 6 L -4 -4 L 4 -4 Z" fill={r.color} stroke="#fff" strokeWidth="1.5" /></g> : <circle key={r.id} cx={(p.x/WORLD_SCALE + 125) * .7} cy={(p.z/WORLD_SCALE + 95) * .7} r={2.7} fill={r.color} />; })}</svg></div>
      <div className="transform-status" role="status">{telemetry.rush>0?`ROCKET RUSH · ${telemetry.rush<=1?'TAKE CONTROL!':Math.ceil(telemetry.rush)+'s'}`:telemetry.form==='plane'?`PLANE · Q CLIMB / F DIVE · ${Math.round(telemetry.altitude)}m`:telemetry.form==='hover'?'HOVERCRAFT · SKIM THE INLET':telemetry.magnet>0?'MAGNET ACTIVE':''}</div>
    ...
    <div className="track-caption"><span><i /> {courseId === 'beach' && racing ? `NEXT: ${telemetry.nextCheckpoint === 0 ? 'START / FINISH' : `CHECKPOINT ${telemetry.nextCheckpoint}`} · EXPLORE THE SAND, FOLLOW THE GATES` : track.sectors.slice(0, 3).join(' → ')}</span><span>3 LAPS / 8 RACERS</span></div>
  </div>;
}
