...
import { beachHeight, WORLD_SCALE } from './beach';
...
export function buildBeachScene(scene: THREE.Scene, track: RaceCourse) {
  ...
  const sun=new THREE.DirectionalLight('#ffe2ac',2.5);sun.position.set(-70,130,-80);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-250,right:250,top:250,bottom:-250,near:1,far:400});sun.shadow.normalBias=.1;scene.add(sun);
  ...
  const geo=new THREE.PlaneGeometry(320*WORLD_SCALE,320*WORLD_SCALE,180,180);geo.rotateX(-Math.PI/2);geo.translate(0,0,20*WORLD_SCALE);
  ...
  for(let i=0;i<38;i++){
    const a=i*2.399,r= (i%3===0?38:118)*WORLD_SCALE,x=Math.cos(a)*r,z=20*WORLD_SCALE+Math.sin(a)*r,y=beachHeight(x,z);
  ...
  for(let i=0;i<8;i++){
    const x=(-55+i*15)*WORLD_SCALE,z=-93*WORLD_SCALE,y=beachHeight(x,z);box(x,y+2,z,9,4,8,['#ef997c','#88c7c3','#d5afd5'][i%3]);
  ...
  for(let i=0;i<14;i++){
    const x=(-55+(i%7)*16)*WORLD_SCALE,z=(40+Math.floor(i/7)*22)*WORLD_SCALE,y=beachHeight(x,z);
  ...
  for(let i=0;i<90;i++){const a=i/90*Math.PI*2,r=125.2*WORLD_SCALE;const f=object(new THREE.SphereGeometry(1,6,4),'#d8f7eb',Math.cos(a)*r,.08,20*WORLD_SCALE+Math.sin(a)*r);f.scale.set(2.1,.06,.6);f.rotation.y=-a;f.castShadow=false;foam.push(f);}
